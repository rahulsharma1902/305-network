                ...
                <div class="bio-hd-dropdwn d-flex align-items-center g-10">
                    ...
                    <div class="col_input">
                        <div class="info_search ms-auto d-flex">
                            <div class="info_search ms-auto d-flex">
                                <select id="coachSelect" class="montserrat">
                                    @if(isset($team->players) && $team->players->count())
                                        @foreach($team->players as $playerData)
                                            <option value="{{ url('sports') ?? '' }}/{{ $team->sport->slug }}/teams/{{ $team->slug }}/players/{{ $playerData->player->slug ?? '' }}">
                                                {{ $playerData->player->first_name ?? '' }} {{ $playerData->player->last_name ?? '' }}
                                            </option>
                                        @endforeach
                                    @endif
                                </select>
                                <div class="i_search" id="searchIcon">
                ...
                @if($player->banner_image_video)
                    <div class="bio-vdo-wrp">
                        ...
                        <div class="video_wrapper">
                            <div class="video-box video_show">
                                @php
                                    $videoUrls = explode(',', $player->banner_image_video ?? '');
                                @endphp
                                ...
                                @foreach ($videoUrls as $videoUrl)
                                    ...
                                    @if ($videoUrl)
                                        @if (Str::endsWith($videoUrl, '.mp4'))
                                            <!-- MP4 Video -->
                                            <video id="videoPlayer" width="100%" height="610" controls>
                                                <source src="{{ $videoUrl }}" type="video/mp4">
                                                Your browser does not support the video tag.
                                            </video>
                                        @else
                                            <!-- YouTube / Vimeo -->
                                            <iframe id="iframePlayer" src="{{ $videoUrl }}" frameborder="0" allowfullscreen loading="lazy" width="100%" height="315"></iframe>
                                        @endif
                                    @endif
                                @endforeach
                            </div>
                        ...
                        <script>
                            document.addEventListener("DOMContentLoaded", function() {
                                ...
                                playButton.addEventListener("click", function() {
                                    ...
                                    if (iframePlayer) {
                                        let src = iframePlayer.src;
                                        if (src.includes("youtube.com")) {
                                            iframePlayer.src = src + (src.includes("?") ? "&" : "?") + "autoplay=1";
                                        } else if (src.includes("vimeo.com")) {
                                            iframePlayer.src = src + (src.includes("?") ? "&" : "?") + "autoplay=1&muted=0";
                                        }
                                    }
                                });
                            });
                            ...
                            <div class="vdo-btm-innr d-flex align-items-center">
                                ...
                                <div class="social-links">
                                    <ul class="list-unstyled d-flex align-items-center m-0">
                                        <li><a href="">{{ $player->email ?? '' }}</a></li>
                                        <li><a href="{{ $player->facebook ?? '' }}"><i class="fa-brands fa-instagram"></i></a></li>
                                        <li><a href="{{ $player->twitter ?? '' }}"><i class="fa-brands fa-x-twitter"></i></a></li>
                                    </ul>
                ...
                <div class="player-details d-flex">
                    ...
                    <div class="plyer-detail-cntnt">
                        <ul class="list-unstyled m-0">
                            <li><span>Position:</span> {{ $player->position->name ?? '' }}</li>
                            <li><span>Height & Weight:</span> {{ $player->height ?? '' }} | {{ $player->weight ?? '' }}</li>
                            <li><span>Jersey Number:</span> {{ $player->jersey_number ?? '' }}</li>
                            <li><span>Age:</span> {{ \Carbon\Carbon::parse($player->dob)->age }}</li>

                            <li><span>Graduation Year:</span> {{ $player->graduation_year ?? '' }}</li>
                            <li><span>High School:</span> {{ $player->high_school ?? '' }}</li>
                            <!-- <li><span>Student ID:</span>#{{ $player->student_id ?? '' }}</li> -->
                        </ul>
                    </div>
                </div>
                    ...
                    <div class="tab-content" id="nav-tabContent">
                        <div class="tab-pane fade show active" id="pills-Athletic" role="tabpanel"
                            ...
                            <div class="tab-inner">
                                ...
                                <p class="m-0">
                                    <?php echo $player->athletic_info ?? ''; ?>
                                </p>

                                <div class="athletic-list">
                                    <ul class="list-unstyled m-0 d-flex">
                                        <li><span>40-Yard Dash Time: </span>{{ $player->yard_dash_time ? $player->yard_dash_time . ' seconds' : 'U/D' }}</li>
                                        <li><span>Shuttle Run Time: </span>{{ $player->shuttle_run_time ? $player->shuttle_run_time . ' seconds' : 'U/D' }}</li>
                                        <li><span>Deadlift Max: </span>{{ $player->deadlift_max ? $player->deadlift_max . ' lbs' : 'U/D' }}</li>
                                    </ul>

                                    <ul class="athletic-list2 list-unstyled m-0 d-flex">
                                        <li><span>Vertical Jump: </span>{{ $player->vertical_jump ? $player->vertical_jump . ' inches' : 'U/D' }}</li>
                                        <li><span>Squat Max: </span>{{ $player->squat_max ? $player->squat_max . ' lbs' : 'U/D' }}</li>
                                    </ul>
                        ...
                        <div class="tab-pane fade" id="pills-Season" role="tabpanel"
                            ...
                            <div class="tab-inner">
                                <h6>Season Stats</h6>
                                <p class="m-0">
                                    <?php echo $player->season_stat ?? ''; ?>
                                </p>

                                <div class="athletic-list">
                                    <ul class="list-unstyled m-0 d-flex">
                                        <!-- Games Played -->
                                        <li><span>Games Played:</span> {{ $player->games_played ?? 'N/A' }}</li>

                                        <!-- Total Yards -->
                                        <li><span>Total Yards:</span> 
                                            @if($player->total_yards_passing || $player->total_yards_rushing || $player->total_yards_receiving)
                                                {{ ($player->total_yards_passing ?? 0) + ($player->total_yards_rushing ?? 0) + ($player->total_yards_receiving ?? 0) }} yards
                                            @else
                                                N/A
                                            @endif
                                        </li>
                                        ...
                                        <!-- Touchdowns -->
                                        <li><span>Touchdowns:</span> {{ $player->touchdowns ?? 'N/A' }}</li>

                                        <!-- Tackles/Sacks -->
                                        <li><span>Tackles/Sacks:</span> {{ $player->tackles ?? 'N/A' }} / {{ $player->sacks ?? 'N/A' }}</li>

                                        <!-- Interceptions -->
                                        <li><span>Interceptions:</span> {{ $player->interceptions ?? 'N/A' }}</li>

                                        <!-- Kicking Stats (if applicable) -->
                                        <li><span>Kicking Stats (if applicable):</span> {{ $player->kicking_stats ?? 'N/A' }}</li>
                                    </ul>
                                </div>
                        ...
                        <div class="tab-pane fade" id="pills-Academic" role="tabpanel"
                            ...
                            <div class="tab-inner">
                                ...
                                <p class="m-0">
                                    <?php echo $player->academic_info ?? ''; ?>
                                </p>

                                    <div class="athletic-list">
                                        <ul class="list-unstyled m-0 d-flex">
                                            <!-- GPA -->
                                            <li><span>GPA: </span> {{ $player->gpa ?? 'N/A' }}</li>

                                            <!-- SAT/ACT Scores (Optional) -->
                                            <li><span>SAT/ACT Scores (Optional): </span> 
                                                {{ $player->sat_act_scores ?? 'N/A' }}
                                            </li>

                                            <!-- Academic Honors -->
                                            <li><span>Academic Honors: </span> 
                                                {{ $player->academic_honors ?? 'N/A' }}
                                            </li>
                                        </ul>
                                    </div>
                            ...
                            <div class="tab-inner">
                                ...
                                <div class="athletic-list">
                                    <ul class="list-unstyled m-0 d-flex">
                                        ...
                                        <!-- Game Footage Link -->
                                        <li><span>Game Footage Link: </span>
                                            @if($player->game_footage_link)
                                                <a href="{{ $player->game_footage_link }}" target="_blank">Watch Game</a>
                                            @else
                                                N/A
                                            @endif
                                        </li>
                                        ...
                                        <!-- Photo Gallery Link -->
                                        <li><span>Photo Gallery Link: </span>
                                            @if($player->photo_gallery_link)
                                                <a href="{{ $player->photo_gallery_link }}" target="_blank">View Gallery</a>
                                            @else
                                                N/A
                                            @endif
                                        </li>
                            ...
                            <div class="tab-inner">
                                ...
                                <p class="m-0">
                                    ...
                                    <div class="athletic-list">
                                        <ul class="list-unstyled m-0 d-flex">
                                            <!-- Team MVP Awards -->
                                            <li><span>Team MVP Awards: </span>
                                                {{ $player->team_mvp_award ?? 'N/A' }}
                                            </li>

                                            <!-- All-State/All-Conference Honors -->
                                            <li><span>All-State/All-Conference Honors: </span>
                                                {{ $player->all_state_honors ?? 'N/A' }}
                                            </li>

                                            <!-- Athletic Camps Attended -->
                                            <li><span>Athletic Camps Attended: </span>
                                                {{ $player->athletic_camps_attended ?? 'N/A' }}
                                            </li>
                                        </ul>
                                    </div>
                        ...
                        <div class="tab-pane fade" id="pills-Recruiting" role="tabpanel"
                            ...
                            <div class="tab-inner">
                                ...
                                <p class="m-0">
                                    ...
                                    <?php echo $player->recruting_info ?? ''; ?>
                                    ...
                                    <div class="athletic-list">
                                        <ul class="list-unstyled m-0 d-flex">
                                            <!-- Commitment Status -->
                                            <li><span>Commitment Status: </span>
                                                {{ $player->commitment_status ?? 'N/A' }}
                                            </li>

                                            <!-- Offers Received -->
                                            <li><span>Offers Received: </span>
                                                @if($player->offers_received && count($player->offers_received) > 0)
                                                    <ul>
                                                        @foreach($player->offers_received as $offer)
                                                            <li>{{ $offer }}</li>
                                                        @endforeach
                                                    </ul>
                                                @else
                                            ...
                                            <!-- Preferred Colleges -->
                                            <li><span>Preferred Colleges: </span>
                                                @if($player->preferred_colleges && count($player->preferred_colleges) > 0)
                                                    <ul>
                                                        @foreach($player->preferred_colleges as $college)
                                                            <li>{{ $college }}</li>
                                                        @endforeach
                                                    </ul>
                                                @else
